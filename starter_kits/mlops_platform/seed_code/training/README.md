...
`ml_pipelines/` contains the SageMaker pipeline definitions.
...
We provide here an example with the Abalone dataset, to train an XGBoost model and evaluate it on a test set before sending it for manual approval to SageMaker Model Registry inside the SageMaker ModelPackageGroup defined when creating the SageMaker Project.
