...
Module configs frequently consume values produced by other modules via SSM references (`ssm-org:`, `ssm-domain:`, `{{resolve:ssm:...}}`, `domainConfigSSMParam`, `generated-role-id:`). See the dynamic references section in [user-config-authoring.md](user-config-authoring.md) for the full standard.
