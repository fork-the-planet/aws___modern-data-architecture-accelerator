...
| -------------------------------------------------------------------- | ------- | ------ | ---------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [dataSources](#dataSources )                                       | No      | object | No         | -                                                   | Nested map of data source type to data source configurations for QuickSight project data connectivity.<br />Outer key is the data source type (e.g., REDSHIFT, ATHENA), inner key is a unique data source ID.<br />Supports credential pairs, secret ARN references, VPC connections, and SSL configuration.<br /><br />Use cases: Redshift data source setup; Athena connectivity; Multi-source BI integration<br /><br />AWS: QuickSight data sources with credentials, VPC connections, and type-specific parameters<br /><br />Validation: Optional; nested map of type → ID → DataSourceProps                                                                                                              |
...
| + [principals](#principals )                                         | No      | object | No         | -                                                   | Named QuickSight principals (users or groups) referenced in folder and data source permissions.<br />Each key is a logical name, value is the QuickSight principal ARN.<br /><br />Use cases: Principal-based access control; Group permission assignment; User management<br /><br />AWS: QuickSight user/group ARNs for permission assignment<br /><br />Validation: Required; map of string keys to QuickSight principal ARNs                                                                                                                                                                                                                                                                                |
| - [resourceAccessRolePermissions](#resourceAccessRolePermissions )   | No      | object | No         | In #/definitions/ResourceAccessRolePermissionsProps | Optional S3/KMS permissions to attach to QuickSight's account-level resource-access role<br />(created by the quicksight-account module) so this project's data sources can read the data<br />they query. Because the queried resources (e.g. the Athena results bucket and its KMS key)<br />are created by modules that deploy before this one, the grants are attached here on the<br />consumer side rather than by quicksight-account.<br /><br />Use cases: Granting QuickSight Athena data sources S3/KMS access to data lake and results buckets<br /><br />AWS: IAM S3/KMS permissions on the QuickSight resource-access role<br /><br />Validation: Optional; see ResourceAccessRolePermissionsProps |
...
| - [sharedFolders](#sharedFolders )                                   | No      | object | No         | -                                                   | Named shared folder configurations for collaborative QuickSight workspace management.<br />Each folder supports hierarchical sub-folders and principal-based permissions<br />(READER_FOLDER or AUTHOR_FOLDER actions).<br /><br />Use cases: Team-based BI workspaces; Dev/test/prod folder separation; Asset organization<br /><br />AWS: QuickSight shared folders with hierarchical structure and permission management<br /><br />Validation: Optional; map of string keys to SharedFoldersProps                                                                                                                                                                                                           |
...
| **Type**                  | `object`                                                                                 |
| **Required**              | No                                                                                       |
| **Additional properties** | [Each additional property must conform to the schema](#dataSources_additionalProperties) |

**Description:** Nested map of data source type to data source configurations for QuickSight project data connectivity.
...
| **Type**                  | `object`                                                                                                      |
| **Required**              | No                                                                                                            |
| **Additional properties** | [Each additional property must conform to the schema](#dataSources_additionalProperties_additionalProperties) |

| Property                                                      | Pattern | Type   | Deprecated | Definition                       | Title/Description                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------------------------------------- | ------- | ------ | ---------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [](#dataSources_additionalProperties_additionalProperties ) | No      | object | No         | In #/definitions/DataSourceProps | Core data source configuration for QuickSight, defining connection parameters,<br />credentials, permissions, and optional SSL/VPC settings.<br /><br />Use cases: Redshift data source; Athena data source; S3/RDS connectivity<br /><br />AWS: QuickSight CfnDataSource<br /><br />Validation: dataSourceSpecificParameters, displayName, and permissions required |
...
**Description:** Core data source configuration for QuickSight, defining connection parameters,
...
| - [secretsManager](#dataSources_additionalProperties_additionalProperties_secretsManager )                             | No      | object | No         | In #/definitions/SecretsManagerAuthProps    | Secrets Manager authentication for the data source. When set, MDAA both (a) wires the<br />secret as the data source credentials, and (b) grants QuickSight's account-level Secrets<br />Manager role (\`aws-quicksight-secretsmanager-role-v0\`) read access to the secret (and<br />decrypt on its KMS key). This is the single toggle for the secret-based auth path — the<br />role name and all IAM wiring are handled automatically.<br /><br />Use cases: Secret-based Redshift (or other credential-pair) data source authentication<br /><br />AWS: QuickSight DataSourceCredentials.SecretArn + IAM grant on the QuickSight SM role<br /><br />Validation: Optional; when present, \`arn\` is required |
...
| + [password](#dataSources_additionalProperties_additionalProperties_credentials_credentialPair_password )                                           | No      | string | No         | -          | Password for data source authentication. Can use dynamic references<br />(e.g., resolve:secretsmanager) for secure credential retrieval.<br /><br />Use cases: Database password; Secret manager dynamic reference<br /><br />AWS: QuickSight data source credential<br /><br />Validation: Required; non-empty string |
| + [username](#dataSources_additionalProperties_additionalProperties_credentials_credentialPair_username )                                           | No      | string | No         | -          | Username for data source authentication.<br /><br />Use cases: Database user identity; Service account access<br /><br />AWS: QuickSight data source credential<br /><br />Validation: Required; non-empty string                                                                                                      |
...
| **Type**                  | `object`                                                                                                                                                                                             |
| **Required**              | No                                                                                                                                                                                                   |
| **Additional properties** | [Each additional property must conform to the schema](#dataSources_additionalProperties_additionalProperties_credentials_credentialPair_alternateDataSourceParameters_items_i0_additionalProperties) |
...
**Description:** Password for data source authentication. Can use dynamic references
...
| **Type**                  | `object`                                                                                                                                                        |
| **Required**              | Yes                                                                                                                                                             |
| **Additional properties** | [Each additional property must conform to the schema](#dataSources_additionalProperties_additionalProperties_dataSourceSpecificParameters_additionalProperties) |
...
| Each item of this array must be                                                                        | Description                                                                                     |
| ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| [DataSourcePermissionsProps](#dataSources_additionalProperties_additionalProperties_permissions_items) | Permission entry for a QuickSight data source, mapping a principal to a data source action. ... |

###### <a name="dataSources_additionalProperties_additionalProperties_permissions_items"></a>1.1.1.6.1. root > dataSources > additionalProperties > additionalProperties > permissions > DataSourcePermissionsProps
...
**Description:** Permission entry for a QuickSight data source, mapping a principal to a data source action.
...
##### <a name="dataSources_additionalProperties_additionalProperties_secretsManager"></a>1.1.1.7. Property `root > dataSources > additionalProperties > additionalProperties > secretsManager`
...
| **Required**              | No                                    |
| **Additional properties** | Not allowed                           |
| **Defined in**            | #/definitions/SecretsManagerAuthProps |

**Description:** Secrets Manager authentication for the data source. When set, MDAA both (a) wires the
...
| Property                                                                                          | Pattern | Type            | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------------------------------------------------------------------- | ------- | --------------- | ---------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| + [arn](#dataSources_additionalProperties_additionalProperties_secretsManager_arn )               | No      | string          | No         | -          | ARN of the Secrets Manager secret holding the data source credentials. Used both as the<br />QuickSight data source credential and to scope the IAM read grant on the QuickSight<br />Secrets Manager role.<br /><br />Use cases: Redshift service-user credentials secret<br /><br />AWS: Secrets Manager secret ARN<br /><br />Validation: Required                                         |
| - [kmsKeyArns](#dataSources_additionalProperties_additionalProperties_secretsManager_kmsKeyArns ) | No      | array of string | No         | -          | KMS key ARNs encrypting the secret. QuickSight's Secrets Manager role is granted decrypt<br />on these so it can read a customer-managed-key-encrypted secret.<br /><br />Use cases: Decrypting a CMK-encrypted credentials secret<br /><br />AWS: IAM kms:Decrypt permission on the QuickSight Secrets Manager role<br /><br />Validation: Optional; required only for CMK-encrypted secrets |

###### <a name="dataSources_additionalProperties_additionalProperties_secretsManager_arn"></a>1.1.1.7.1. Property `root > dataSources > additionalProperties > additionalProperties > secretsManager > arn`
...
**Description:** ARN of the Secrets Manager secret holding the data source credentials. Used both as the
...
###### <a name="dataSources_additionalProperties_additionalProperties_secretsManager_kmsKeyArns"></a>1.1.1.7.2. Property `root > dataSources > additionalProperties > additionalProperties > secretsManager > kmsKeyArns`
...
**Description:** KMS key ARNs encrypting the secret. QuickSight's Secrets Manager role is granted decrypt
...
| Each item of this array must be                                                                            | Description |
| ---------------------------------------------------------------------------------------------------------- | ----------- |
| [kmsKeyArns items](#dataSources_additionalProperties_additionalProperties_secretsManager_kmsKeyArns_items) | -           |

###### <a name="dataSources_additionalProperties_additionalProperties_secretsManager_kmsKeyArns_items"></a>1.1.1.7.2.1. root > dataSources > additionalProperties > additionalProperties > secretsManager > kmsKeyArns > kmsKeyArns items
...
##### <a name="dataSources_additionalProperties_additionalProperties_sslProperties"></a>1.1.1.8. Property `root > dataSources > additionalProperties > additionalProperties > sslProperties`
...
###### <a name="dataSources_additionalProperties_additionalProperties_sslProperties_disableSsl"></a>1.1.1.8.1. Property `root > dataSources > additionalProperties > additionalProperties > sslProperties > disableSsl`
...
##### <a name="dataSources_additionalProperties_additionalProperties_vpcConnectionProperties"></a>1.1.1.9. Property `root > dataSources > additionalProperties > additionalProperties > vpcConnectionProperties`
...
###### <a name="dataSources_additionalProperties_additionalProperties_vpcConnectionProperties_vpcConnectionArn"></a>1.1.1.9.1. Property `root > dataSources > additionalProperties > additionalProperties > vpcConnectionProperties > vpcConnectionArn`
...
**Description:** Named QuickSight principals (users or groups) referenced in folder and data source permissions.
...
## <a name="resourceAccessRolePermissions"></a>4. Property `root > resourceAccessRolePermissions`
...
**Description:** Optional S3/KMS permissions to attach to QuickSight's account-level resource-access role
...
| Property                                                       | Pattern | Type            | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                                                                                                                                                                  |
| -------------------------------------------------------------- | ------- | --------------- | ---------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| - [kmsKeyArns](#resourceAccessRolePermissions_kmsKeyArns )     | No      | array of string | No         | -          | KMS key ARNs the data source must use to decrypt KMS-encrypted data and query results.<br /><br />Use cases: Decrypting KMS-encrypted data lake objects and Athena results<br /><br />AWS: IAM KMS permissions on the QuickSight resource-access role<br /><br />Validation: Optional; array of KMS key ARNs                                                                                                       |
| - [s3BucketArns](#resourceAccessRolePermissions_s3BucketArns ) | No      | array of string | No         | -          | S3 bucket ARNs the data source must read (and write, for Athena query results). Grants<br />List/Get on the buckets and their objects, plus Put for query results.<br /><br />Use cases: Data lake bucket access; Athena query-results bucket access<br /><br />AWS: IAM S3 permissions on the QuickSight resource-access role<br /><br />Validation: Optional; array of S3 bucket ARNs (arn:aws:s3:::bucket-name) |

### <a name="resourceAccessRolePermissions_kmsKeyArns"></a>4.1. Property `root > resourceAccessRolePermissions > kmsKeyArns`
...
**Description:** KMS key ARNs the data source must use to decrypt KMS-encrypted data and query results.
...
| Each item of this array must be                                     | Description |
| ------------------------------------------------------------------- | ----------- |
| [kmsKeyArns items](#resourceAccessRolePermissions_kmsKeyArns_items) | -           |

#### <a name="resourceAccessRolePermissions_kmsKeyArns_items"></a>4.1.1. root > resourceAccessRolePermissions > kmsKeyArns > kmsKeyArns items
...
### <a name="resourceAccessRolePermissions_s3BucketArns"></a>4.2. Property `root > resourceAccessRolePermissions > s3BucketArns`
...
**Description:** S3 bucket ARNs the data source must read (and write, for Athena query results). Grants
...
| Each item of this array must be                                         | Description |
| ----------------------------------------------------------------------- | ----------- |
| [s3BucketArns items](#resourceAccessRolePermissions_s3BucketArns_items) | -           |

#### <a name="resourceAccessRolePermissions_s3BucketArns_items"></a>4.2.1. root > resourceAccessRolePermissions > s3BucketArns > s3BucketArns items
...
###### <a name="sagemakerBlueprint_additionalAccounts_additionalProperties_provisioningRole_arn"></a>5.1.1.5.1. Property `root > sagemakerBlueprint > additionalAccounts > additionalProperties > provisioningRole > arn`
...
###### <a name="sagemakerBlueprint_additionalAccounts_additionalProperties_provisioningRole_id"></a>5.1.1.5.2. Property `root > sagemakerBlueprint > additionalAccounts > additionalProperties > provisioningRole > id`
...
###### <a name="sagemakerBlueprint_additionalAccounts_additionalProperties_provisioningRole_immutable"></a>5.1.1.5.3. Property `root > sagemakerBlueprint > additionalAccounts > additionalProperties > provisioningRole > immutable`
...
###### <a name="sagemakerBlueprint_additionalAccounts_additionalProperties_provisioningRole_name"></a>5.1.1.5.4. Property `root > sagemakerBlueprint > additionalAccounts > additionalProperties > provisioningRole > name`
...
###### <a name="sagemakerBlueprint_additionalAccounts_additionalProperties_provisioningRole_refId"></a>5.1.1.5.5. Property `root > sagemakerBlueprint > additionalAccounts > additionalProperties > provisioningRole > refId`
...
###### <a name="sagemakerBlueprint_additionalAccounts_additionalProperties_provisioningRole_sso"></a>5.1.1.5.6. Property `root > sagemakerBlueprint > additionalAccounts > additionalProperties > provisioningRole > sso`
...
## <a name="sharedFolders"></a>7. Property `root > sharedFolders`

|                           |                                                                                            |
| ------------------------- | ------------------------------------------------------------------------------------------ |
| **Type**                  | `object`                                                                                   |
| **Required**              | No                                                                                         |
| **Additional properties** | [Each additional property must conform to the schema](#sharedFolders_additionalProperties) |

**Description:** Named shared folder configurations for collaborative QuickSight workspace management.
...
| Property                                   | Pattern | Type   | Deprecated | Definition                          | Title/Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------ | ------- | ------ | ---------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [](#sharedFolders_additionalProperties ) | No      | object | No         | In #/definitions/SharedFoldersProps | Shared folder configuration with permissions and optional nested sub-folders.<br />Folders organize QuickSight assets (dashboards, analyses, datasets) with<br />hierarchical structure and per-folder access control.<br /><br />Use cases: Environment-based folder hierarchy (dev/test/prod); Team workspace isolation; Asset organization<br /><br />AWS: QuickSight shared folders via custom resource<br /><br />Validation: permissions required; folders optional for nesting |

### <a name="sharedFolders_additionalProperties"></a>7.1. Property `root > sharedFolders > SharedFoldersProps`
...
**Description:** Shared folder configuration with permissions and optional nested sub-folders.
...
| Property                                                          | Pattern | Type   | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------------------------------------------------- | ------- | ------ | ---------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [folders](#sharedFolders_additionalProperties_folders )         | No      | object | No         | -          | Nested child folders inheriting the parent's organizational context.<br />Each child folder has its own permissions and can contain further sub-folders.<br /><br />Use cases: Working/publishing sub-folder separation; Hierarchical asset organization<br /><br />AWS: QuickSight nested shared folders<br /><br />Validation: Optional; map of folder name to SharedFoldersProps |
| + [permissions](#sharedFolders_additionalProperties_permissions ) | No      | array  | No         | -          | Permission entries controlling who can access this folder and at what level.<br /><br />Use cases: Multi-group access control; Reader/Author separation per folder<br /><br />AWS: QuickSight folder permissions<br /><br />Validation: Required; array of SharedFoldersPermissionsProps                                                                                            |

#### <a name="sharedFolders_additionalProperties_folders"></a>7.1.1. Property `root > sharedFolders > additionalProperties > folders`

|                           |                                                                                                                         |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| **Type**                  | `object`                                                                                                                |
| **Required**              | No                                                                                                                      |
| **Additional properties** | [Each additional property must conform to the schema](#sharedFolders_additionalProperties_folders_additionalProperties) |

**Description:** Nested child folders inheriting the parent's organizational context.
...
| Property                                                                | Pattern | Type   | Deprecated | Definition                                                                         | Title/Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------------------------------------------------------- | ------- | ------ | ---------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [](#sharedFolders_additionalProperties_folders_additionalProperties ) | No      | object | No         | Same as [sharedFolders_additionalProperties](#sharedFolders_additionalProperties ) | Shared folder configuration with permissions and optional nested sub-folders.<br />Folders organize QuickSight assets (dashboards, analyses, datasets) with<br />hierarchical structure and per-folder access control.<br /><br />Use cases: Environment-based folder hierarchy (dev/test/prod); Team workspace isolation; Asset organization<br /><br />AWS: QuickSight shared folders via custom resource<br /><br />Validation: permissions required; folders optional for nesting |

##### <a name="sharedFolders_additionalProperties_folders_additionalProperties"></a>7.1.1.1. Property `root > sharedFolders > additionalProperties > folders > SharedFoldersProps`
...
#### <a name="sharedFolders_additionalProperties_permissions"></a>7.1.2. Property `root > sharedFolders > additionalProperties > permissions`
...
**Description:** Permission entries controlling who can access this folder and at what level.
...
| Each item of this array must be                                                        | Description                                                                                  |
| -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| [SharedFoldersPermissionsProps](#sharedFolders_additionalProperties_permissions_items) | Permission entry for a QuickSight shared folder, mapping a principal to a folder action. ... |

##### <a name="sharedFolders_additionalProperties_permissions_items"></a>7.1.2.1. root > sharedFolders > additionalProperties > permissions > SharedFoldersPermissionsProps
...
**Description:** Permission entry for a QuickSight shared folder, mapping a principal to a folder action.
...
| Property                                                                        | Pattern | Type             | Deprecated | Definition                     | Title/Description                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------- | ------- | ---------------- | ---------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| + [actions](#sharedFolders_additionalProperties_permissions_items_actions )     | No      | enum (of string) | No         | In #/definitions/FolderActions | Folder permission level granted to the principal.<br /><br />Use cases: Read-only dashboard consumers; Full authoring access for BI developers<br /><br />AWS: QuickSight folder actions (DescribeFolder for READER, full CRUD for AUTHOR)<br /><br />Validation: Required; 'READER_FOLDER' \| 'AUTHOR_FOLDER'     |
| + [principal](#sharedFolders_additionalProperties_permissions_items_principal ) | No      | string           | No         | -                              | Principal name referencing a QuickSight user or group from the project's principals map.<br /><br />Use cases: Group-based folder access; User-specific permissions<br /><br />AWS: QuickSight principal (user or group)<br /><br />Validation: Required; must match a key in the project principals configuration |

###### <a name="sharedFolders_additionalProperties_permissions_items_actions"></a>7.1.2.1.1. Property `root > sharedFolders > additionalProperties > permissions > permissions items > actions`
...
**Description:** Folder permission level granted to the principal.
...
###### <a name="sharedFolders_additionalProperties_permissions_items_principal"></a>7.1.2.1.2. Property `root > sharedFolders > additionalProperties > permissions > permissions items > principal`
...
**Description:** Principal name referencing a QuickSight user or group from the project's principals map.
