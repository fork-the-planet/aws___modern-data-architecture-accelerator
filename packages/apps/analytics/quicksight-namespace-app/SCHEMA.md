...
| -------------------------------------------------------------------- | ------- | --------------- | ---------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| + [federations](#federations )                                       | No      | object          | No         | -                                                   | Named federation configurations for identity provider integration with QuickSight namespaces.<br />Each federation creates IAM roles for SAML-based access, QuickSight namespace, users, and groups.<br />Roles are configured with QS user types (READER/AUTHOR) and group memberships.<br /><br />Use cases: SAML federation setup; Multi-tenant QuickSight namespaces; Group-based QS access<br /><br />AWS: QuickSight namespaces with IAM SAML federation roles and automated user/group management<br /><br />Validation: Required; map of string keys to FederationProps                                                                                                         |
| - [glueResourceAccess](#glueResourceAccess )                         | No      | array of string | No         | -                                                   | Glue resource names (database/table patterns) to which namespace roles are granted IAM read access.<br />Used for QuickSight data source setup and validation.<br /><br />Use cases: Glue catalog data source access; Database/table pattern-based permissions<br /><br />AWS: IAM Glue resource access for QuickSight namespace roles<br /><br />Validation: Optional; array of Glue resource name patterns (e.g., 'database/my-db*')                                                                                                                                                                                                                                                  |
...
**Description:** Named federation configurations for identity provider integration with QuickSight namespaces.
...
| **Type**                  | `object`                                                                                                            |
| **Required**              | Yes                                                                                                                 |
| **Additional properties** | [Each additional property must conform to the schema](#federations_additionalProperties_roles_additionalProperties) |

**Description:** QS Groups and QS Role(Reader|Author) info for creating IAM Roles, Creating QS Groups, Registering Users with a QS Role
...
| **Required**              | No                                |
| **Additional properties** | Not allowed                       |
| **Defined in**            | #/definitions/FederationRoleProps |

**Description:** Role configuration mapping QuickSight groups to a user type (READER or AUTHOR)
...
**Description:** QS Groups info for creating Creating QS Groups
...
| **Type**       | `enum (of string)`       |
| **Required**   | Yes                      |
| **Defined in** | #/definitions/QSUserType |

**Description:** QS Role(Reader|Author) info for creating IAM Roles
...
**Description:** URL used by the connecting driver

## <a name="glueResourceAccess"></a>2. Property `root > glueResourceAccess`
...
**Description:** Glue resource names (database/table patterns) to which namespace roles are granted IAM read access.
...
| Each item of this array must be                       | Description |
| ----------------------------------------------------- | ----------- |
| [glueResourceAccess items](#glueResourceAccess_items) | -           |

### <a name="glueResourceAccess_items"></a>2.1. root > glueResourceAccess > glueResourceAccess items
