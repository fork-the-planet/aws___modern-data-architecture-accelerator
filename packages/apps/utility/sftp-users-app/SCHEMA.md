...
| -------------------------------------------------------------------- | ------- | ------ | ---------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| + [buckets](#buckets )                                               | No      | object | No         | -                                                   | Map of bucket names to S3 bucket configurations with KMS encryption.<br />Referenced by users in the bucket field for home directory storage.<br /><br />Use cases: Centralized bucket configuration; Shared storage across users<br /><br />AWS: S3 buckets with KMS encryption for Transfer Family user storage<br /><br />Validation: Required; keys are unique names, values must be valid BucketConfig                                                                                                                                                                                                                                                                             |
...
| + [publicKeys](#publicKeys )                                         | No      | object | No         | -                                                   | Map of key names to SSH public key configurations.<br />Referenced by users in the publicKeys array for authentication.<br /><br />Use cases: Centralized SSH key management; Key reuse across multiple users<br /><br />AWS: Transfer Family SSH public keys for user authentication<br /><br />Validation: Required; keys are unique names, values must be valid PublicKeyConfig                                                                                                                                                                                                                                                                                                      |
...
| + [serverId](#serverId )                                             | No      | string | No         | -                                                   | Transfer Family server ID to associate users with.<br />Accepts server IDs or SSM parameter references.<br /><br />Use cases: Linking users to a deployed SFTP server<br /><br />AWS: Transfer Family server ID for user association<br /><br />Validation: Required; must be valid Transfer Family server ID or SSM parameter path                                                                                                                                                                                                                                                                                                                                                     |
...
| + [users](#users )                                                   | No      | object | No         | -                                                   | Map of user names to SFTP user configurations.<br />Each user references entries from publicKeys and buckets sections.<br />If no accessRoleArn is specified, MDAA creates a minimally-scoped IAM role.<br /><br />Use cases: SFTP user provisioning; Per-user home directory and key assignment<br /><br />AWS: Transfer Family users with S3 home directories and SSH authentication<br /><br />Validation: Required; keys are unique user names, values must be valid UserConfig                                                                                                                                                                                                     |
...
**Description:** Map of bucket names to S3 bucket configurations with KMS encryption.
...
| Property                             | Pattern | Type   | Deprecated | Definition                    | Title/Description                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------ | ------- | ------ | ---------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| - [](#buckets_additionalProperties ) | No      | object | No         | In #/definitions/BucketConfig | S3 bucket configuration for SFTP user home directories.<br />Files uploaded via SFTP are stored in this bucket with KMS encryption.<br /><br />Use cases: SFTP home directory storage; Encrypted file storage for SFTP operations<br /><br />AWS: S3 bucket with KMS encryption for Transfer Family user file storage<br /><br />Validation: bucketName and kmsKeyArn required |
...
**Description:** S3 bucket configuration for SFTP user home directories.
...
| Property                                                  | Pattern | Type   | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                                                                                                              |
| --------------------------------------------------------- | ------- | ------ | ---------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| + [bucketName](#buckets_additionalProperties_bucketName ) | No      | string | No         | -          | S3 bucket name for SFTP user file storage. Accepts bucket names<br />or SSM parameter references.<br /><br />Use cases: SFTP home directory bucket; Cross-account bucket access<br /><br />AWS: S3 bucket for Transfer Family user home directory<br /><br />Validation: Required; must be valid S3 bucket name or SSM parameter path                          |
| + [kmsKeyArn](#buckets_additionalProperties_kmsKeyArn )   | No      | string | No         | -          | KMS key ARN for encrypting files uploaded via SFTP.<br />Accepts key ARNs or SSM parameter references.<br /><br />Use cases: Data-at-rest encryption for SFTP uploads; Compliance requirements<br /><br />AWS: KMS key for S3 object encryption in Transfer Family operations<br /><br />Validation: Required; must be valid KMS key ARN or SSM parameter path |
...
**Description:** S3 bucket name for SFTP user file storage. Accepts bucket names
...
**Description:** KMS key ARN for encrypting files uploaded via SFTP.
...
| **Type**                  | `object`                                                                                |
| **Required**              | Yes                                                                                     |
| **Additional properties** | [Each additional property must conform to the schema](#publicKeys_additionalProperties) |

**Description:** Map of key names to SSH public key configurations.
...
| Property                                | Pattern | Type   | Deprecated | Definition                       | Title/Description                                                                                                                                                                                                                                                                                                                     |
| --------------------------------------- | ------- | ------ | ---------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [](#publicKeys_additionalProperties ) | No      | object | No         | In #/definitions/PublicKeyConfig | SSH public key for SFTP user authentication.<br />Private keys are managed externally.<br /><br />Use cases: SSH key-based SFTP authentication; Password-less secure access<br /><br />AWS: Transfer Family user SSH public key<br /><br />Validation: publicKey required; must be valid SSH public key format (e.g. ssh-rsa AAAA...) |
...
**Description:** SSH public key for SFTP user authentication.
...
| Property                                                   | Pattern | Type   | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                                                                                           |
| ---------------------------------------------------------- | ------- | ------ | ---------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| + [publicKey](#publicKeys_additionalProperties_publicKey ) | No      | string | No         | -          | SSH public key contents for user authentication.<br />Corresponding private key must be managed externally.<br /><br />Use cases: SSH key-based SFTP login; Secure password-less authentication<br /><br />AWS: Transfer Family user SSH public key for authentication<br /><br />Validation: Required; must be valid SSH public key format |
...
**Description:** SSH public key contents for user authentication.
...
## <a name="serverId"></a>5. Property `root > serverId`
...
**Description:** Transfer Family server ID to associate users with.
...
## <a name="users"></a>7. Property `root > users`

|                           |                                                                                    |
| ------------------------- | ---------------------------------------------------------------------------------- |
| **Type**                  | `object`                                                                           |
| **Required**              | Yes                                                                                |
| **Additional properties** | [Each additional property must conform to the schema](#users_additionalProperties) |

**Description:** Map of user names to SFTP user configurations.
...
| Property                           | Pattern | Type   | Deprecated | Definition                  | Title/Description                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------------------------------- | ------- | ------ | ---------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [](#users_additionalProperties ) | No      | object | No         | In #/definitions/UserConfig | SFTP user configuration linking authentication keys, S3 storage, and home directory.<br />References named entries from the publicKeys and buckets config sections.<br /><br />Use cases: SFTP user provisioning; Home directory mapping; Multi-key authentication<br /><br />AWS: Transfer Family user with S3 home directory and SSH key authentication<br /><br />Validation: bucket, homeDirectory, and publicKeys required |

### <a name="users_additionalProperties"></a>7.1. Property `root > users > UserConfig`
...
**Description:** SFTP user configuration linking authentication keys, S3 storage, and home directory.
...
| Property                                                      | Pattern | Type            | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------- | ------- | --------------- | ---------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| - [accessRoleArn](#users_additionalProperties_accessRoleArn ) | No      | string          | No         | -          | Existing IAM role ARN for user S3 access. Accepts role ARNs or SSM parameter references.<br />If omitted, MDAA creates a minimally-scoped role automatically.<br /><br />Use cases: Custom access permissions; Pre-existing role reuse; Cross-account access<br /><br />AWS: IAM role for Transfer Family user S3 and KMS access<br /><br />Validation: Optional; must be valid IAM role ARN or SSM parameter path |
| + [bucket](#users_additionalProperties_bucket )               | No      | string          | No         | -          | Name of a bucket entry from the buckets config section.<br />Links this user to a specific S3 bucket for home directory storage.<br /><br />Use cases: User-to-bucket mapping; Shared bucket configuration across users<br /><br />AWS: Transfer Family user home directory bucket reference<br /><br />Validation: Required; must reference a valid key from the buckets config                                   |
| + [homeDirectory](#users_additionalProperties_homeDirectory ) | No      | string          | No         | -          | S3 prefix for the user's home directory within the configured bucket<br />(e.g. /incoming).<br /><br />Use cases: User directory isolation; Organized file storage per user<br /><br />AWS: Transfer Family user home directory S3 prefix<br /><br />Validation: Required; valid S3 prefix string                                                                                                                  |
| + [publicKeys](#users_additionalProperties_publicKeys )       | No      | array of string | No         | -          | Names of public key entries from the publicKeys config section.<br />Supports multiple keys for key rotation and multi-device access.<br /><br />Use cases: Multi-key authentication; Key rotation support<br /><br />AWS: Transfer Family user SSH public key references<br /><br />Validation: Required; array of valid key names from publicKeys config                                                         |

#### <a name="users_additionalProperties_accessRoleArn"></a>7.1.1. Property `root > users > additionalProperties > accessRoleArn`
...
**Description:** Existing IAM role ARN for user S3 access. Accepts role ARNs or SSM parameter references.
...
#### <a name="users_additionalProperties_bucket"></a>7.1.2. Property `root > users > additionalProperties > bucket`
...
**Description:** Name of a bucket entry from the buckets config section.
...
#### <a name="users_additionalProperties_homeDirectory"></a>7.1.3. Property `root > users > additionalProperties > homeDirectory`
...
**Description:** S3 prefix for the user's home directory within the configured bucket
(e.g. /incoming).
...
#### <a name="users_additionalProperties_publicKeys"></a>7.1.4. Property `root > users > additionalProperties > publicKeys`
...
**Description:** Names of public key entries from the publicKeys config section.
...
##### <a name="users_additionalProperties_publicKeys_items"></a>7.1.4.1. root > users > additionalProperties > publicKeys > publicKeys items
