...
| + [crawlers](#crawlers )                                             | No      | object | No         | -                                                   | Map of crawler names to Glue crawler definitions for data source discovery and cataloging.<br /><br />Use cases: Multi-source data discovery; Catalog population; Schema detection<br /><br />AWS: AWS Glue crawlers<br /><br />Validation: Required; map of string to CrawlerDefinition                                                                                                                                                                                                                                                                                                                                                                                                |
...
| - [projectName](#projectName )                                       | No      | string | No         | -                                                   | DataOps project name for crawler resource autowiring (databases, roles, security).<br /><br />Use cases: Project integration; Shared infrastructure reuse<br /><br />AWS: DataOps project reference<br /><br />Validation: Optional; must match an existing deployed project                                                                                                                                                                                                                                                                                                                                                                                                            |
...
## <a name="crawlers"></a>2. Property `root > crawlers`

|                           |                                                                                       |
| ------------------------- | ------------------------------------------------------------------------------------- |
| **Type**                  | `object`                                                                              |
| **Required**              | Yes                                                                                   |
| **Additional properties** | [Each additional property must conform to the schema](#crawlers_additionalProperties) |

**Description:** Map of crawler names to Glue crawler definitions for data source discovery and cataloging.
...
| Property                              | Pattern | Type   | Deprecated | Definition                         | Title/Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------------------------------- | ------- | ------ | ---------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [](#crawlers_additionalProperties ) | No      | object | No         | In #/definitions/CrawlerDefinition | Configuration for a Glue crawler including execution role, targets, scheduling, and schema management.<br /><br />Use cases: Automated S3 data discovery, scheduled database cataloging, schema change detection, data lake metadata management<br /><br />AWS: AWS Glue crawler configuration for automated data discovery and Glue catalog population<br /><br />Validation: executionRoleArn, databaseName, description, and targets are required; schedule, schemaChangePolicy, and tablePrefix are optional |

### <a name="crawlers_additionalProperties"></a>2.1. Property `root > crawlers > CrawlerDefinition`
...
**Description:** Configuration for a Glue crawler including execution role, targets, scheduling, and schema management.
...
| - [schedule](#crawlers_additionalProperties_schedule )                     | No      | object          | No         | In #/definitions/CfnCrawler.ScheduleProperty           | Cron or rate schedule for automated periodic crawler execution.<br /><br />Use cases: Automated data discovery, scheduled catalog updates, periodic schema detection<br /><br />AWS: AWS Glue crawler schedule configuration<br /><br />Validation: Must be valid CfnCrawler.ScheduleProperty if provided                       |
| - [schemaChangePolicy](#crawlers_additionalProperties_schemaChangePolicy ) | No      | object          | No         | In #/definitions/CfnCrawler.SchemaChangePolicyProperty | Policy controlling how the crawler handles detected schema modifications.<br /><br />Use cases: Schema evolution management, table structure change handling, metadata consistency<br /><br />AWS: AWS Glue crawler schema change policy<br /><br />Validation: Must be valid CfnCrawler.SchemaChangePolicyProperty if provided |
| - [tablePrefix](#crawlers_additionalProperties_tablePrefix )               | No      | string          | No         | -                                                      | String prefix prepended to all table names created by the crawler.<br /><br />Use cases: Table naming organization, namespace management, naming conflict avoidance<br /><br />AWS: AWS Glue crawler table prefix<br /><br />Validation: Must be a valid string if provided                                                     |
| + [targets](#crawlers_additionalProperties_targets )                       | No      | object          | No         | In #/definitions/CrawlerTargets                        | Crawler targets specifying data sources (S3, JDBC, DynamoDB, catalog) to crawl.<br /><br />Use cases: Data source specification, multi-source crawling, comprehensive data discovery<br /><br />AWS: AWS Glue crawler targets configuration<br /><br />Validation: Must be a valid CrawlerTargets object; required              |

#### <a name="crawlers_additionalProperties_classifiers"></a>2.1.1. Property `root > crawlers > additionalProperties > classifiers`
...
##### <a name="crawlers_additionalProperties_classifiers_items"></a>2.1.1.1. root > crawlers > additionalProperties > classifiers > classifiers items
...
#### <a name="crawlers_additionalProperties_databaseName"></a>2.1.2. Property `root > crawlers > additionalProperties > databaseName`
...
#### <a name="crawlers_additionalProperties_description"></a>2.1.3. Property `root > crawlers > additionalProperties > description`
...
#### <a name="crawlers_additionalProperties_executionRoleArn"></a>2.1.4. Property `root > crawlers > additionalProperties > executionRoleArn`
...
#### <a name="crawlers_additionalProperties_extraConfiguration"></a>2.1.5. Property `root > crawlers > additionalProperties > extraConfiguration`

|                           |                                                                                                                               |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **Type**                  | `object`                                                                                                                      |
| **Required**              | No                                                                                                                            |
| **Additional properties** | [Each additional property must conform to the schema](#crawlers_additionalProperties_extraConfiguration_additionalProperties) |
...
##### <a name="crawlers_additionalProperties_extraConfiguration_additionalProperties"></a>2.1.5.1. Property `root > crawlers > additionalProperties > extraConfiguration > additionalProperties`
...
#### <a name="crawlers_additionalProperties_recrawlBehavior"></a>2.1.6. Property `root > crawlers > additionalProperties > recrawlBehavior`
...
#### <a name="crawlers_additionalProperties_schedule"></a>2.1.7. Property `root > crawlers > additionalProperties > schedule`
...
**Description:** Cron or rate schedule for automated periodic crawler execution.
...
##### <a name="crawlers_additionalProperties_schedule_scheduleExpression"></a>2.1.7.1. Property `root > crawlers > additionalProperties > schedule > scheduleExpression`
...
#### <a name="crawlers_additionalProperties_schemaChangePolicy"></a>2.1.8. Property `root > crawlers > additionalProperties > schemaChangePolicy`
...
**Description:** Policy controlling how the crawler handles detected schema modifications.
...
##### <a name="crawlers_additionalProperties_schemaChangePolicy_deleteBehavior"></a>2.1.8.1. Property `root > crawlers > additionalProperties > schemaChangePolicy > deleteBehavior`
...
##### <a name="crawlers_additionalProperties_schemaChangePolicy_updateBehavior"></a>2.1.8.2. Property `root > crawlers > additionalProperties > schemaChangePolicy > updateBehavior`
...
#### <a name="crawlers_additionalProperties_tablePrefix"></a>2.1.9. Property `root > crawlers > additionalProperties > tablePrefix`
...
**Description:** String prefix prepended to all table names created by the crawler.
...
#### <a name="crawlers_additionalProperties_targets"></a>2.1.10. Property `root > crawlers > additionalProperties > targets`
...
**Description:** Crawler targets specifying data sources (S3, JDBC, DynamoDB, catalog) to crawl.
...
| Property                                                                     | Pattern | Type  | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------------------- | ------- | ----- | ---------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [catalogTargets](#crawlers_additionalProperties_targets_catalogTargets )   | No      | array | No         | -          | Glue catalog targets for discovering and synchronizing existing catalog metadata.<br /><br />Use cases: Cross-catalog synchronization, existing table discovery, metadata consolidation<br /><br />AWS: AWS Glue crawler catalog targets<br /><br />Validation: Must be array of valid CfnCrawler.CatalogTargetProperty if provided |
| - [dynamoDbTargets](#crawlers_additionalProperties_targets_dynamoDbTargets ) | No      | array | No         | -          | DynamoDB targets for NoSQL table schema discovery and catalog integration.<br /><br />Use cases: DynamoDB schema discovery, NoSQL table cataloging, cross-service data integration<br /><br />AWS: AWS Glue crawler DynamoDB targets<br /><br />Validation: Must be array of valid CfnCrawler.DynamoDBTargetProperty if provided    |
| - [jdbcTargets](#crawlers_additionalProperties_targets_jdbcTargets )         | No      | array | No         | -          | JDBC targets for relational database schema discovery and table cataloging.<br /><br />Use cases: Relational database discovery, JDBC schema cataloging, database-to-lake integration<br /><br />AWS: AWS Glue crawler JDBC targets<br /><br />Validation: Must be array of valid CfnCrawler.JdbcTargetProperty if provided         |
| - [s3Targets](#crawlers_additionalProperties_targets_s3Targets )             | No      | array | No         | -          | S3 targets for data lake object discovery and automatic schema inference.<br /><br />Use cases: S3 data lake discovery, automatic schema inference, partition discovery<br /><br />AWS: AWS Glue crawler S3 targets<br /><br />Validation: Must be array of valid CfnCrawler.S3TargetProperty if provided                           |

##### <a name="crawlers_additionalProperties_targets_catalogTargets"></a>2.1.10.1. Property `root > crawlers > additionalProperties > targets > catalogTargets`
...
**Description:** Glue catalog targets for discovering and synchronizing existing catalog metadata.
...
###### <a name="crawlers_additionalProperties_targets_catalogTargets_items"></a>2.1.10.1.1. root > crawlers > additionalProperties > targets > catalogTargets > CfnCrawler.CatalogTargetProperty
...
###### <a name="crawlers_additionalProperties_targets_catalogTargets_items_connectionName"></a>2.1.10.1.1.1. Property `root > crawlers > additionalProperties > targets > catalogTargets > catalogTargets items > connectionName`
...
###### <a name="crawlers_additionalProperties_targets_catalogTargets_items_databaseName"></a>2.1.10.1.1.2. Property `root > crawlers > additionalProperties > targets > catalogTargets > catalogTargets items > databaseName`
...
###### <a name="crawlers_additionalProperties_targets_catalogTargets_items_dlqEventQueueArn"></a>2.1.10.1.1.3. Property `root > crawlers > additionalProperties > targets > catalogTargets > catalogTargets items > dlqEventQueueArn`
...
###### <a name="crawlers_additionalProperties_targets_catalogTargets_items_eventQueueArn"></a>2.1.10.1.1.4. Property `root > crawlers > additionalProperties > targets > catalogTargets > catalogTargets items > eventQueueArn`
...
###### <a name="crawlers_additionalProperties_targets_catalogTargets_items_tables"></a>2.1.10.1.1.5. Property `root > crawlers > additionalProperties > targets > catalogTargets > catalogTargets items > tables`
...
###### <a name="crawlers_additionalProperties_targets_catalogTargets_items_tables_items"></a>2.1.10.1.1.5.1. root > crawlers > additionalProperties > targets > catalogTargets > catalogTargets items > tables > tables items
...
##### <a name="crawlers_additionalProperties_targets_dynamoDbTargets"></a>2.1.10.2. Property `root > crawlers > additionalProperties > targets > dynamoDbTargets`
...
**Description:** DynamoDB targets for NoSQL table schema discovery and catalog integration.
...
###### <a name="crawlers_additionalProperties_targets_dynamoDbTargets_items"></a>2.1.10.2.1. root > crawlers > additionalProperties > targets > dynamoDbTargets > CfnCrawler.DynamoDBTargetProperty
...
| - [scanAll](#crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll )   | No      | Combination | No         | -          | Indicates whether to scan all the records, or to sample rows from the table.<br /><br />Scanning all the records can take a long time when the table is not a high throughput table.<br /><br />A value of \`true\` means to scan all records, while a value of \`false\` means to sample the records. If no value is specified, the value defaults to \`true\` .                                                                                                                                                                                                               |
| - [scanRate](#crawlers_additionalProperties_targets_dynamoDbTargets_items_scanRate ) | No      | number      | No         | -          | The percentage of the configured read capacity units to use by the AWS Glue crawler.<br /><br />Read capacity units is a term defined by DynamoDB, and is a numeric value that acts as rate limiter for the number of reads that can be performed on that table per second.<br /><br />The valid values are null or a value between 0.1 to 1.5. A null value is used when user does not provide a value, and defaults to 0.5 of the configured Read Capacity Unit (for provisioned tables), or 0.25 of the max configured Read Capacity Unit (for tables using on-demand mode). |

###### <a name="crawlers_additionalProperties_targets_dynamoDbTargets_items_path"></a>2.1.10.2.1.1. Property `root > crawlers > additionalProperties > targets > dynamoDbTargets > dynamoDbTargets items > path`
...
###### <a name="crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll"></a>2.1.10.2.1.2. Property `root > crawlers > additionalProperties > targets > dynamoDbTargets > dynamoDbTargets items > scanAll`
...
**Description:** Indicates whether to scan all the records, or to sample rows from the table.
...
| Any of(Option)                                                                               |
| -------------------------------------------------------------------------------------------- |
| [IResolvable](#crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0) |
| [item 1](#crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i1)      |

###### <a name="crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0"></a>2.1.10.2.1.2.1. Property `root > crawlers > additionalProperties > targets > dynamoDbTargets > dynamoDbTargets items > scanAll > anyOf > IResolvable`
...
| Property                                                                                                        | Pattern | Type             | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                            |
| --------------------------------------------------------------------------------------------------------------- | ------- | ---------------- | ---------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| + [creationStack](#crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0_creationStack ) | No      | array of string  | No         | -          | The creation stack of this resolvable which will be appended to errors<br />thrown during resolution.<br /><br />This may return an array with a single informational element indicating how<br />to get this property populated, if it was skipped for performance reasons. |
| - [typeHint](#crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0_typeHint )           | No      | enum (of string) | No         | -          | The type that this token will likely resolve to.                                                                                                                                                                                                                             |

###### <a name="crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0_creationStack"></a>2.1.10.2.1.2.1.1. Property `root > crawlers > additionalProperties > targets > dynamoDbTargets > dynamoDbTargets items > scanAll > anyOf > item 0 > creationStack`
...
| Each item of this array must be                                                                                          | Description |
| ------------------------------------------------------------------------------------------------------------------------ | ----------- |
| [creationStack items](#crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0_creationStack_items) | -           |

###### <a name="crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0_creationStack_items"></a>2.1.10.2.1.2.1.1.1. root > crawlers > additionalProperties > targets > dynamoDbTargets > dynamoDbTargets items > scanAll > anyOf > item 0 > creationStack > creationStack items
...
###### <a name="crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0_typeHint"></a>2.1.10.2.1.2.1.2. Property `root > crawlers > additionalProperties > targets > dynamoDbTargets > dynamoDbTargets items > scanAll > anyOf > item 0 > typeHint`
...
###### <a name="crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i1"></a>2.1.10.2.1.2.2. Property `root > crawlers > additionalProperties > targets > dynamoDbTargets > dynamoDbTargets items > scanAll > anyOf > item 1`
...
###### <a name="crawlers_additionalProperties_targets_dynamoDbTargets_items_scanRate"></a>2.1.10.2.1.3. Property `root > crawlers > additionalProperties > targets > dynamoDbTargets > dynamoDbTargets items > scanRate`
...
**Description:** The percentage of the configured read capacity units to use by the AWS Glue crawler.
...
##### <a name="crawlers_additionalProperties_targets_jdbcTargets"></a>2.1.10.3. Property `root > crawlers > additionalProperties > targets > jdbcTargets`
...
**Description:** JDBC targets for relational database schema discovery and table cataloging.
...
###### <a name="crawlers_additionalProperties_targets_jdbcTargets_items"></a>2.1.10.3.1. root > crawlers > additionalProperties > targets > jdbcTargets > CfnCrawler.JdbcTargetProperty
...
###### <a name="crawlers_additionalProperties_targets_jdbcTargets_items_connectionName"></a>2.1.10.3.1.1. Property `root > crawlers > additionalProperties > targets > jdbcTargets > jdbcTargets items > connectionName`
...
###### <a name="crawlers_additionalProperties_targets_jdbcTargets_items_enableAdditionalMetadata"></a>2.1.10.3.1.2. Property `root > crawlers > additionalProperties > targets > jdbcTargets > jdbcTargets items > enableAdditionalMetadata`
...
###### <a name="crawlers_additionalProperties_targets_jdbcTargets_items_enableAdditionalMetadata_items"></a>2.1.10.3.1.2.1. root > crawlers > additionalProperties > targets > jdbcTargets > jdbcTargets items > enableAdditionalMetadata > enableAdditionalMetadata items
...
###### <a name="crawlers_additionalProperties_targets_jdbcTargets_items_exclusions"></a>2.1.10.3.1.3. Property `root > crawlers > additionalProperties > targets > jdbcTargets > jdbcTargets items > exclusions`
...
###### <a name="crawlers_additionalProperties_targets_jdbcTargets_items_exclusions_items"></a>2.1.10.3.1.3.1. root > crawlers > additionalProperties > targets > jdbcTargets > jdbcTargets items > exclusions > exclusions items
...
###### <a name="crawlers_additionalProperties_targets_jdbcTargets_items_path"></a>2.1.10.3.1.4. Property `root > crawlers > additionalProperties > targets > jdbcTargets > jdbcTargets items > path`
...
##### <a name="crawlers_additionalProperties_targets_s3Targets"></a>2.1.10.4. Property `root > crawlers > additionalProperties > targets > s3Targets`
...
**Description:** S3 targets for data lake object discovery and automatic schema inference.
...
###### <a name="crawlers_additionalProperties_targets_s3Targets_items"></a>2.1.10.4.1. root > crawlers > additionalProperties > targets > s3Targets > CfnCrawler.S3TargetProperty
...
###### <a name="crawlers_additionalProperties_targets_s3Targets_items_connectionName"></a>2.1.10.4.1.1. Property `root > crawlers > additionalProperties > targets > s3Targets > s3Targets items > connectionName`
...
###### <a name="crawlers_additionalProperties_targets_s3Targets_items_dlqEventQueueArn"></a>2.1.10.4.1.2. Property `root > crawlers > additionalProperties > targets > s3Targets > s3Targets items > dlqEventQueueArn`
...
###### <a name="crawlers_additionalProperties_targets_s3Targets_items_eventQueueArn"></a>2.1.10.4.1.3. Property `root > crawlers > additionalProperties > targets > s3Targets > s3Targets items > eventQueueArn`
...
###### <a name="crawlers_additionalProperties_targets_s3Targets_items_exclusions"></a>2.1.10.4.1.4. Property `root > crawlers > additionalProperties > targets > s3Targets > s3Targets items > exclusions`
...
###### <a name="crawlers_additionalProperties_targets_s3Targets_items_exclusions_items"></a>2.1.10.4.1.4.1. root > crawlers > additionalProperties > targets > s3Targets > s3Targets items > exclusions > exclusions items
...
###### <a name="crawlers_additionalProperties_targets_s3Targets_items_path"></a>2.1.10.4.1.5. Property `root > crawlers > additionalProperties > targets > s3Targets > s3Targets items > path`
...
###### <a name="crawlers_additionalProperties_targets_s3Targets_items_sampleSize"></a>2.1.10.4.1.6. Property `root > crawlers > additionalProperties > targets > s3Targets > s3Targets items > sampleSize`
...
**Description:** DataOps project name for crawler resource autowiring (databases, roles, security).
...
| + [renderedTags](#sagemakerBlueprint_domainConfig_domainConfigCr_handlerFunction__connections_securityGroups_items_stack_tags_renderedTags )               | No      | object | No         | Same as [crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0](#crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0 ) | A lazy value that represents the rendered tags at synthesis time<br /><br />If you need to make a custom construct taggable, use the value of this<br />property to pass to the \`tags\` property of the underlying construct. |
...
| **Required**              | Yes                                                                                                                                                           |
| **Additional properties** | Not allowed                                                                                                                                                   |
| **Same definition as**    | [crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0](#crawlers_additionalProperties_targets_dynamoDbTargets_items_scanAll_anyOf_i0) |
