...
| - [dataAdminRoles](#dataAdminRoles )                                 | No      | array  | No         | -                                                   | Data admin roles granted the cluster access managed policy for ALL Aurora clusters.<br />These roles receive rds-db:connect (IAM auth), rds:DescribeDBClusters, and<br />Secrets Manager access to every cluster's admin secret.<br /><br />Use cases: Platform admin access; Cross-cluster administration; Security management<br /><br />AWS: IAM roles with cluster access managed policies attached for all clusters<br /><br />Validation: Optional; array of valid MdaaRoleRef                                                                                                                                                                                                    |
...
| - [postgresql](#postgresql )                                         | No      | object | No         | -                                                   | Map of named Aurora PostgreSQL Serverless v2 cluster configurations.<br />Each key is the cluster name (used as the cluster identifier after MDAA naming).<br />Deploys KMS-encrypted, VPC-bound clusters with enhanced monitoring,<br />IAM authentication, and automatic admin password rotation.<br /><br />Use cases: Relational database provisioning; PostgreSQL workloads; Multi-cluster deployment<br /><br />AWS: Aurora PostgreSQL Serverless v2 clusters<br /><br />Validation: Optional; map of cluster names to valid AuroraPostgresqlClusterProps                                                                                                                         |
...
## <a name="dataAdminRoles"></a>2. Property `root > dataAdminRoles`
...
**Description:** Data admin roles granted the cluster access managed policy for ALL Aurora clusters.
...
### <a name="dataAdminRoles_items"></a>2.1. root > dataAdminRoles > MdaaRoleRef
...
#### <a name="dataAdminRoles_items_arn"></a>2.1.1. Property `root > dataAdminRoles > dataAdminRoles items > arn`
...
#### <a name="dataAdminRoles_items_id"></a>2.1.2. Property `root > dataAdminRoles > dataAdminRoles items > id`
...
#### <a name="dataAdminRoles_items_immutable"></a>2.1.3. Property `root > dataAdminRoles > dataAdminRoles items > immutable`
...
#### <a name="dataAdminRoles_items_name"></a>2.1.4. Property `root > dataAdminRoles > dataAdminRoles items > name`
...
#### <a name="dataAdminRoles_items_refId"></a>2.1.5. Property `root > dataAdminRoles > dataAdminRoles items > refId`
...
#### <a name="dataAdminRoles_items_sso"></a>2.1.6. Property `root > dataAdminRoles > dataAdminRoles items > sso`
...
## <a name="postgresql"></a>7. Property `root > postgresql`

|                           |                                                                                         |
| ------------------------- | --------------------------------------------------------------------------------------- |
| **Type**                  | `object`                                                                                |
| **Required**              | No                                                                                      |
| **Additional properties** | [Each additional property must conform to the schema](#postgresql_additionalProperties) |

**Description:** Map of named Aurora PostgreSQL Serverless v2 cluster configurations.
...
| Property                                | Pattern | Type   | Deprecated | Definition                                    | Title/Description                                     |
| --------------------------------------- | ------- | ------ | ---------- | --------------------------------------------- | ----------------------------------------------------- |
| - [](#postgresql_additionalProperties ) | No      | object | No         | In #/definitions/AuroraPostgresqlClusterProps | Configuration for a single Aurora PostgreSQL cluster. |

### <a name="postgresql_additionalProperties"></a>7.1. Property `root > postgresql > AuroraPostgresqlClusterProps`
...
| **Required**              | No                                         |
| **Additional properties** | Not allowed                                |
| **Defined in**            | #/definitions/AuroraPostgresqlClusterProps |

**Description:** Configuration for a single Aurora PostgreSQL cluster.

| Property                                                                                       | Pattern | Type    | Deprecated | Definition                                       | Title/Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---------------------------------------------------------------------------------------------- | ------- | ------- | ---------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [adminPasswordRotationDays](#postgresql_additionalProperties_adminPasswordRotationDays )     | No      | number  | No         | -                                                | Days between automatic admin/master password rotation via Secrets Manager.<br /><br />Use cases: Automated credential rotation; Security compliance; Password policy enforcement<br /><br />AWS: Secrets Manager automatic rotation schedule<br /><br />Validation: Optional; positive integer                                                                                                                                                                                                                                                                                                                        |
| - [backupRetentionDays](#postgresql_additionalProperties_backupRetentionDays )                 | No      | number  | No         | -                                                | Number of days to retain automated backups (1-35).<br /><br />Use cases: Point-in-time recovery; Data protection compliance; Backup management<br /><br />AWS: Aurora automated backup retention period<br /><br />Validation: Optional; integer 1-35                                                                                                                                                                                                                                                                                                                                                                 |
| - [clusterAccessRoles](#postgresql_additionalProperties_clusterAccessRoles )                   | No      | array   | No         | -                                                | Roles granted access to this cluster via a managed policy. The policy grants<br />rds-db:connect (IAM auth), rds:DescribeDBClusters, and Secrets Manager access<br />to the cluster admin secret.<br /><br />Use cases: Application access; Developer access; Service integration<br /><br />AWS: IAM roles with Aurora cluster access managed policy attached<br /><br />Validation: Optional; array of valid MdaaRoleRef                                                                                                                                                                                            |
| - [dataAdminRoles](#postgresql_additionalProperties_dataAdminRoles )                           | No      | array   | No         | -                                                | Admin roles granted access to the Aurora KMS encryption key for key management.<br />Not required when using a project KMS key (projectName is set).<br /><br />Use cases: Key administration; Security management; Encryption key access control<br /><br />AWS: IAM roles with KMS key admin permissions<br /><br />Validation: Optional; array of valid MdaaRoleRef                                                                                                                                                                                                                                                |
| - [defaultDatabaseName](#postgresql_additionalProperties_defaultDatabaseName )                 | No      | string  | No         | -                                                | Initial database name created in the cluster.<br /><br />Use cases: Application database setup; Custom database naming; Initial provisioning<br /><br />AWS: Aurora initial database name<br /><br />Validation: Optional; valid PostgreSQL database name                                                                                                                                                                                                                                                                                                                                                             |
| - [enableCloudwatchLogsExports](#postgresql_additionalProperties_enableCloudwatchLogsExports ) | No      | boolean | No         | -                                                | Enable export of PostgreSQL logs to CloudWatch Logs for monitoring and analysis.<br /><br />Use cases: Operational monitoring; Query auditing; Troubleshooting<br /><br />AWS: Aurora PostgreSQL log export to CloudWatch Logs<br /><br />Validation: Optional; boolean                                                                                                                                                                                                                                                                                                                                               |
| - [enableDataApi](#postgresql_additionalProperties_enableDataApi )                             | No      | boolean | No         | -                                                | Enable the RDS Data API for HTTP-based SQL access without persistent connections.<br /><br />Use cases: Lambda integration; Serverless access; HTTP-based queries<br /><br />AWS: Aurora Data API (RDS Data Service)<br /><br />Validation: Optional; boolean                                                                                                                                                                                                                                                                                                                                                         |
| - [enableIamAuthentication](#postgresql_additionalProperties_enableIamAuthentication )         | No      | boolean | No         | -                                                | Enable IAM database authentication for token-based access without passwords.<br /><br />Use cases: IAM-based access control; Passwordless authentication; Fine-grained access<br /><br />AWS: Aurora IAM database authentication<br /><br />Validation: Optional; boolean                                                                                                                                                                                                                                                                                                                                             |
| + [engineVersion](#postgresql_additionalProperties_engineVersion )                             | No      | string  | No         | -                                                | Aurora PostgreSQL engine version string in major.minor format.<br /><br />Use cases: Version pinning; Feature availability; Upgrade planning<br /><br />AWS: Aurora PostgreSQL engine version<br /><br />Validation: Required; valid Aurora PostgreSQL version string (e.g., '16.13')                                                                                                                                                                                                                                                                                                                                 |
| - [maxCapacity](#postgresql_additionalProperties_maxCapacity )                                 | No      | number  | No         | -                                                | Maximum Aurora Serverless v2 capacity units (ACUs). The cluster scales up to this<br />capacity under load.<br /><br />Use cases: Performance ceiling; Cost control; Burst capacity<br /><br />AWS: Aurora Serverless v2 maximum ACU capacity<br /><br />Validation: Optional; number between 1 and 256; must be >= minCapacity                                                                                                                                                                                                                                                                                       |
| - [minCapacity](#postgresql_additionalProperties_minCapacity )                                 | No      | number  | No         | -                                                | Minimum Aurora Serverless v2 capacity units (ACUs). Each ACU provides approximately<br />2 GiB of memory with corresponding CPU and networking.<br /><br />Use cases: Cost optimization; Minimum resource guarantee; Baseline capacity<br /><br />AWS: Aurora Serverless v2 minimum ACU capacity<br /><br />Validation: Optional; number between 0.5 and 256                                                                                                                                                                                                                                                          |
| - [numberOfReaders](#postgresql_additionalProperties_numberOfReaders )                         | No      | number  | No         | -                                                | Number of Aurora reader instances for read scaling and high availability.<br /><br />Use cases: Read scaling; High availability; Failover targets<br /><br />AWS: Aurora read replica instances<br /><br />Validation: Optional; non-negative integer                                                                                                                                                                                                                                                                                                                                                                 |
| + [port](#postgresql_additionalProperties_port )                                               | No      | number  | No         | -                                                | Non-default TCP port for client connections. Port obfuscation adds an additional<br />layer of defense against non-targeted attacks. Avoid using the default PostgreSQL<br />port 5432.<br /><br />Use cases: Port obfuscation; Security hardening; Network policy compliance<br /><br />AWS: Aurora cluster endpoint port<br /><br />Validation: Required; valid port number; should not be 5432                                                                                                                                                                                                                     |
| - [securityGroupId](#postgresql_additionalProperties_securityGroupId )                         | No      | string  | No         | -                                                | Existing security group ID to use for the Aurora cluster instead of creating a new one.<br />When set, the security group is imported and any securityGroupIngress rules are added to it.<br />Supports SSM references (ssm:/path) and project references (project:securityGroupId/<name>)<br />which are auto-resolved when projectName is configured.<br /><br />Use cases: Shared security group from DataOps project; Centralized network policy; Reuse across clusters<br /><br />AWS: Existing EC2 security group<br /><br />Validation: Optional; valid security group ID, SSM reference, or project reference |
| - [securityGroupIngress](#postgresql_additionalProperties_securityGroupIngress )               | No      | object  | No         | In #/definitions/AuroraSecurityGroupIngressProps | Security group ingress rules controlling network access to the Aurora cluster port.<br />When securityGroupId is also provided, these rules are added to the imported security group.<br />Egress is restricted by default (MdaaSecurityGroup denies all outbound unless explicitly configured).<br /><br />Use cases: Network access control; Client connectivity; Security group management<br /><br />AWS: VPC security group ingress rules for Aurora cluster<br /><br />Validation: Optional; object with optional ipv4 and/or sg arrays                                                                         |
| + [subnets](#postgresql_additionalProperties_subnets )                                         | No      | array   | No         | -                                                | Subnet configurations for Aurora cluster node placement. Subnets should span<br />multiple availability zones for high availability.<br /><br />Use cases: Multi-AZ placement; Subnet-specific deployment; High availability<br /><br />AWS: VPC subnets for Aurora DB subnet group<br /><br />Validation: Required; array of AuroraSubnetConfig; subnets must be in the specified VPC                                                                                                                                                                                                                                |
| + [vpcId](#postgresql_additionalProperties_vpcId )                                             | No      | string  | No         | -                                                | VPC ID for Aurora cluster deployment. The cluster is deployed within this VPC<br />with network access controlled by security groups.<br /><br />Use cases: Network isolation; VPC-based deployment; Secure networking<br /><br />AWS: VPC for Aurora cluster network configuration<br /><br />Validation: Required; valid VPC ID                                                                                                                                                                                                                                                                                     |

#### <a name="postgresql_additionalProperties_adminPasswordRotationDays"></a>7.1.1. Property `root > postgresql > additionalProperties > adminPasswordRotationDays`
...
**Description:** Days between automatic admin/master password rotation via Secrets Manager.
...
#### <a name="postgresql_additionalProperties_backupRetentionDays"></a>7.1.2. Property `root > postgresql > additionalProperties > backupRetentionDays`
...
#### <a name="postgresql_additionalProperties_clusterAccessRoles"></a>7.1.3. Property `root > postgresql > additionalProperties > clusterAccessRoles`
...
**Description:** Roles granted access to this cluster via a managed policy. The policy grants
...
| Each item of this array must be                                          | Description |
| ------------------------------------------------------------------------ | ----------- |
| [MdaaRoleRef](#postgresql_additionalProperties_clusterAccessRoles_items) | -           |

##### <a name="postgresql_additionalProperties_clusterAccessRoles_items"></a>7.1.3.1. root > postgresql > additionalProperties > clusterAccessRoles > MdaaRoleRef
...
#### <a name="postgresql_additionalProperties_dataAdminRoles"></a>7.1.4. Property `root > postgresql > additionalProperties > dataAdminRoles`
...
**Description:** Admin roles granted access to the Aurora KMS encryption key for key management.
...
| Each item of this array must be                                      | Description |
| -------------------------------------------------------------------- | ----------- |
| [MdaaRoleRef](#postgresql_additionalProperties_dataAdminRoles_items) | -           |

##### <a name="postgresql_additionalProperties_dataAdminRoles_items"></a>7.1.4.1. root > postgresql > additionalProperties > dataAdminRoles > MdaaRoleRef
...
#### <a name="postgresql_additionalProperties_defaultDatabaseName"></a>7.1.5. Property `root > postgresql > additionalProperties > defaultDatabaseName`
...
#### <a name="postgresql_additionalProperties_enableCloudwatchLogsExports"></a>7.1.6. Property `root > postgresql > additionalProperties > enableCloudwatchLogsExports`
...
**Description:** Enable export of PostgreSQL logs to CloudWatch Logs for monitoring and analysis.
...
#### <a name="postgresql_additionalProperties_enableDataApi"></a>7.1.7. Property `root > postgresql > additionalProperties > enableDataApi`
...
**Description:** Enable the RDS Data API for HTTP-based SQL access without persistent connections.
...
#### <a name="postgresql_additionalProperties_enableIamAuthentication"></a>7.1.8. Property `root > postgresql > additionalProperties > enableIamAuthentication`
...
**Description:** Enable IAM database authentication for token-based access without passwords.
...
#### <a name="postgresql_additionalProperties_engineVersion"></a>7.1.9. Property `root > postgresql > additionalProperties > engineVersion`
...
**Description:** Aurora PostgreSQL engine version string in major.minor format.
...
#### <a name="postgresql_additionalProperties_maxCapacity"></a>7.1.10. Property `root > postgresql > additionalProperties > maxCapacity`
...
**Description:** Maximum Aurora Serverless v2 capacity units (ACUs). The cluster scales up to this
capacity under load.
...
#### <a name="postgresql_additionalProperties_minCapacity"></a>7.1.11. Property `root > postgresql > additionalProperties > minCapacity`
...
**Description:** Minimum Aurora Serverless v2 capacity units (ACUs). Each ACU provides approximately
...
#### <a name="postgresql_additionalProperties_numberOfReaders"></a>7.1.12. Property `root > postgresql > additionalProperties > numberOfReaders`
...
**Description:** Number of Aurora reader instances for read scaling and high availability.
...
#### <a name="postgresql_additionalProperties_port"></a>7.1.13. Property `root > postgresql > additionalProperties > port`
...
**Description:** Non-default TCP port for client connections. Port obfuscation adds an additional
...
#### <a name="postgresql_additionalProperties_securityGroupId"></a>7.1.14. Property `root > postgresql > additionalProperties > securityGroupId`
...
**Description:** Existing security group ID to use for the Aurora cluster instead of creating a new one.
...
#### <a name="postgresql_additionalProperties_securityGroupIngress"></a>7.1.15. Property `root > postgresql > additionalProperties > securityGroupIngress`
...
| **Required**              | No                                            |
| **Additional properties** | Not allowed                                   |
| **Defined in**            | #/definitions/AuroraSecurityGroupIngressProps |

**Description:** Security group ingress rules controlling network access to the Aurora cluster port.
...
| Property                                                              | Pattern | Type            | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                                                       |
| --------------------------------------------------------------------- | ------- | --------------- | ---------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [ipv4](#postgresql_additionalProperties_securityGroupIngress_ipv4 ) | No      | array of string | No         | -          | IPv4 CIDR blocks allowed to connect to the Aurora cluster port.<br /><br />Use cases: VPC CIDR access; On-premises connectivity; Specific IP allowlisting<br /><br />AWS: Security group ingress rules with IPv4 CIDR source<br /><br />Validation: Optional; array of valid IPv4 CIDR blocks           |
| - [sg](#postgresql_additionalProperties_securityGroupIngress_sg )     | No      | array of string | No         | -          | Security group IDs allowed to connect to the Aurora cluster port.<br /><br />Use cases: Application-tier access; Lambda connectivity; ECS/EKS service access<br /><br />AWS: Security group ingress rules with security group source<br /><br />Validation: Optional; array of valid security group IDs |

##### <a name="postgresql_additionalProperties_securityGroupIngress_ipv4"></a>7.1.15.1. Property `root > postgresql > additionalProperties > securityGroupIngress > ipv4`
...
**Description:** IPv4 CIDR blocks allowed to connect to the Aurora cluster port.
...
| Each item of this array must be                                                | Description |
| ------------------------------------------------------------------------------ | ----------- |
| [ipv4 items](#postgresql_additionalProperties_securityGroupIngress_ipv4_items) | -           |

###### <a name="postgresql_additionalProperties_securityGroupIngress_ipv4_items"></a>7.1.15.1.1. root > postgresql > additionalProperties > securityGroupIngress > ipv4 > ipv4 items
...
##### <a name="postgresql_additionalProperties_securityGroupIngress_sg"></a>7.1.15.2. Property `root > postgresql > additionalProperties > securityGroupIngress > sg`
...
**Description:** Security group IDs allowed to connect to the Aurora cluster port.
...
| Each item of this array must be                                            | Description |
| -------------------------------------------------------------------------- | ----------- |
| [sg items](#postgresql_additionalProperties_securityGroupIngress_sg_items) | -           |

###### <a name="postgresql_additionalProperties_securityGroupIngress_sg_items"></a>7.1.15.2.1. root > postgresql > additionalProperties > securityGroupIngress > sg > sg items
...
#### <a name="postgresql_additionalProperties_subnets"></a>7.1.16. Property `root > postgresql > additionalProperties > subnets`
...
**Description:** Subnet configurations for Aurora cluster node placement. Subnets should span
...
| Each item of this array must be                                      | Description                                        |
| -------------------------------------------------------------------- | -------------------------------------------------- |
| [AuroraSubnetConfig](#postgresql_additionalProperties_subnets_items) | Subnet configuration for Aurora cluster placement. |

##### <a name="postgresql_additionalProperties_subnets_items"></a>7.1.16.1. root > postgresql > additionalProperties > subnets > AuroraSubnetConfig
...
| **Required**              | No                               |
| **Additional properties** | Not allowed                      |
| **Defined in**            | #/definitions/AuroraSubnetConfig |

**Description:** Subnet configuration for Aurora cluster placement.

| Property                                                                               | Pattern | Type   | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                       |
| -------------------------------------------------------------------------------------- | ------- | ------ | ---------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| + [availabilityZone](#postgresql_additionalProperties_subnets_items_availabilityZone ) | No      | string | No         | -          | Availability zone of the subnet.<br /><br />Use cases: Multi-AZ distribution; Zone-aware placement; High availability<br /><br />AWS: Availability zone identifier (e.g., us-east-1a)<br /><br />Validation: Required; must match the actual AZ of the specified subnet |
| + [subnetId](#postgresql_additionalProperties_subnets_items_subnetId )                 | No      | string | No         | -          | Subnet ID for cluster node placement.<br /><br />Use cases: Multi-AZ deployment; Network isolation; Subnet-specific placement<br /><br />AWS: VPC subnet for Aurora cluster subnet group<br /><br />Validation: Required; valid subnet ID in the specified VPC          |

###### <a name="postgresql_additionalProperties_subnets_items_availabilityZone"></a>7.1.16.1.1. Property `root > postgresql > additionalProperties > subnets > subnets items > availabilityZone`
...
###### <a name="postgresql_additionalProperties_subnets_items_subnetId"></a>7.1.16.1.2. Property `root > postgresql > additionalProperties > subnets > subnets items > subnetId`
...
**Description:** Subnet ID for cluster node placement.
...
#### <a name="postgresql_additionalProperties_vpcId"></a>7.1.17. Property `root > postgresql > additionalProperties > vpcId`
...
**Description:** VPC ID for Aurora cluster deployment. The cluster is deployed within this VPC
