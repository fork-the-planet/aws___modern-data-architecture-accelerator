...
| -------------------------------------------------------------------- | ------- | ------ | ---------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [accessPolicies](#accessPolicies )                                 | No      | object | No         | -                                                   | Named catalog access policies for fine-grained resource-level access control.<br />Each policy defines read/write principal ARNs scoped to specific catalog resource ARNs<br />(databases, tables, partitions). The Glue Catalog module deploys a KMS-encrypted catalog<br />with resource policies for cross-account data mesh architectures.<br /><br />Use cases: Resource-scoped read/write permissions; Cross-account catalog access; Fine-grained data governance<br /><br />AWS: Glue Catalog resource policies with IAM principal-based access control<br /><br />Validation: Optional; map of string keys to CatalogAccessPolicyProps                                          |
| - [consumerAccounts](#consumerAccounts )                             | No      | object | No         | -                                                   | Consumer accounts granted read access to the entire Glue Catalog via catalog resource policy.<br />Each entry maps a friendly account name to an AWS account ID. Consumer accounts also receive<br />KMS key usage permissions for decrypting catalog metadata.<br /><br />Use cases: Data mesh consumer nodes; Hub/spoke catalog sharing; Cross-account data discovery<br /><br />AWS: Glue Catalog resource policy and KMS key policy grants per consumer account<br /><br />Validation: Optional; map of string keys to 12-digit AWS account IDs                                                                                                                                     |
| - [kmsKeyConsumerAccounts](#kmsKeyConsumerAccounts )                 | No      | object | No         | -                                                   | Accounts granted access to the catalog KMS encryption key only, without catalog read access.<br />Useful when accounts need to decrypt catalog-encrypted data but should not browse the catalog directly.<br /><br />Use cases: Selective encryption key sharing; Decrypt-only cross-account access; Limited key permissions<br /><br />AWS: KMS key policy grants for Glue Catalog encryption key<br /><br />Validation: Optional; map of string keys to 12-digit AWS account IDs                                                                                                                                                                                                      |
...
| - [producerAccounts](#producerAccounts )                             | No      | object | No         | -                                                   | Producer accounts for which additional Athena data source catalogs are created in the deployment account.<br />Each entry maps a friendly account name to the producer's AWS account ID. Does not grant access<br />to the producer catalog unless separately configured on the producer side.<br /><br />Use cases: Data mesh producer nodes; Federated Athena queries across accounts; Multi-catalog analytics<br /><br />AWS: Athena CfnDataCatalog resources pointing to producer account Glue Catalogs<br /><br />Validation: Optional; map of string keys to 12-digit AWS account IDs                                                                                             |
...
**Description:** Named catalog access policies for fine-grained resource-level access control.
...
| Property                                    | Pattern | Type   | Deprecated | Definition                                | Title/Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------------- | ------- | ------ | ---------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [](#accessPolicies_additionalProperties ) | No      | object | No         | In #/definitions/CatalogAccessPolicyProps | Defines a Glue Catalog access policy scoped to specific catalog resources.<br />Principals receive Glue API permissions (Get*, List* for read; same for write)<br />on the specified resource ARNs via catalog resource policy.<br /><br />Use cases: Resource-scoped read/write permissions; Cross-account catalog access; Fine-grained data governance<br /><br />AWS: Glue Catalog resource policy statements with IAM principal-based access<br /><br />Validation: resources required; readPrincipalArns and writePrincipalArns optional |

### <a name="accessPolicies_additionalProperties"></a>1.1. Property `root > accessPolicies > CatalogAccessPolicyProps`
...
| **Required**              | No                                     |
| **Additional properties** | Not allowed                            |
| **Defined in**            | #/definitions/CatalogAccessPolicyProps |

**Description:** Defines a Glue Catalog access policy scoped to specific catalog resources.
...
| Property                                                                         | Pattern | Type            | Deprecated | Definition | Title/Description                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| -------------------------------------------------------------------------------- | ------- | --------------- | ---------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - [readPrincipalArns](#accessPolicies_additionalProperties_readPrincipalArns )   | No      | array of string | No         | -          | IAM principal ARNs granted read-only access (glue:Get*, glue:List*) to the specified catalog resources.<br /><br />Use cases: Cross-account data discovery; Read-only catalog access; Query permissions<br /><br />AWS: Glue Catalog resource policy read permissions<br /><br />Validation: Optional; array of valid IAM principal ARNs                                                                                                          |
| + [resources](#accessPolicies_additionalProperties_resources )                   | No      | array of string | No         | -          | Glue Catalog resource ARNs defining the scope of this access policy.<br />Supports database, table, and partition ARNs for fine-grained resource-level control.<br /><br />Use cases: Database-scoped policies; Table-level access control; Partition-level governance<br /><br />AWS: Glue Catalog resource ARNs (arn:aws:glue:region:account:catalog/database/table)<br /><br />Validation: Required; array of valid Glue catalog resource ARNs |
| - [writePrincipalArns](#accessPolicies_additionalProperties_writePrincipalArns ) | No      | array of string | No         | -          | IAM principal ARNs granted read/write access to the specified catalog resources.<br />Write principals receive the same Glue API permissions as read principals.<br /><br />Use cases: Cross-account catalog management; Schema administration; ETL pipeline permissions<br /><br />AWS: Glue Catalog resource policy write permissions<br /><br />Validation: Optional; array of valid IAM principal ARNs                                        |

#### <a name="accessPolicies_additionalProperties_readPrincipalArns"></a>1.1.1. Property `root > accessPolicies > additionalProperties > readPrincipalArns`
...
**Description:** IAM principal ARNs granted read-only access (glue:Get*, glue:List*) to the specified catalog resources.
...
| --------------------------------------------------------------------------------------- | ----------- |
| [readPrincipalArns items](#accessPolicies_additionalProperties_readPrincipalArns_items) | -           |

##### <a name="accessPolicies_additionalProperties_readPrincipalArns_items"></a>1.1.1.1. root > accessPolicies > additionalProperties > readPrincipalArns > readPrincipalArns items
...
#### <a name="accessPolicies_additionalProperties_resources"></a>1.1.2. Property `root > accessPolicies > additionalProperties > resources`
...
**Description:** Glue Catalog resource ARNs defining the scope of this access policy.
...
| Each item of this array must be                                         | Description |
| ----------------------------------------------------------------------- | ----------- |
| [resources items](#accessPolicies_additionalProperties_resources_items) | -           |

##### <a name="accessPolicies_additionalProperties_resources_items"></a>1.1.2.1. root > accessPolicies > additionalProperties > resources > resources items
...
#### <a name="accessPolicies_additionalProperties_writePrincipalArns"></a>1.1.3. Property `root > accessPolicies > additionalProperties > writePrincipalArns`
...
**Description:** IAM principal ARNs granted read/write access to the specified catalog resources.
...
| Each item of this array must be                                                           | Description |
| ----------------------------------------------------------------------------------------- | ----------- |
| [writePrincipalArns items](#accessPolicies_additionalProperties_writePrincipalArns_items) | -           |

##### <a name="accessPolicies_additionalProperties_writePrincipalArns_items"></a>1.1.3.1. root > accessPolicies > additionalProperties > writePrincipalArns > writePrincipalArns items
...
## <a name="consumerAccounts"></a>2. Property `root > consumerAccounts`

|                           |                                                                                               |
| ------------------------- | --------------------------------------------------------------------------------------------- |
| **Type**                  | `object`                                                                                      |
| **Required**              | No                                                                                            |
| **Additional properties** | [Each additional property must conform to the schema](#consumerAccounts_additionalProperties) |

**Description:** Consumer accounts granted read access to the entire Glue Catalog via catalog resource policy.
...
| Property                                      | Pattern | Type   | Deprecated | Definition | Title/Description |
| --------------------------------------------- | ------- | ------ | ---------- | ---------- | ----------------- |
| - [](#consumerAccounts_additionalProperties ) | No      | string | No         | -          | -                 |

### <a name="consumerAccounts_additionalProperties"></a>2.1. Property `root > consumerAccounts > additionalProperties`
...
## <a name="kmsKeyConsumerAccounts"></a>3. Property `root > kmsKeyConsumerAccounts`

|                           |                                                                                                     |
| ------------------------- | --------------------------------------------------------------------------------------------------- |
| **Type**                  | `object`                                                                                            |
| **Required**              | No                                                                                                  |
| **Additional properties** | [Each additional property must conform to the schema](#kmsKeyConsumerAccounts_additionalProperties) |

**Description:** Accounts granted access to the catalog KMS encryption key only, without catalog read access.
...
| Property                                            | Pattern | Type   | Deprecated | Definition | Title/Description |
| --------------------------------------------------- | ------- | ------ | ---------- | ---------- | ----------------- |
| - [](#kmsKeyConsumerAccounts_additionalProperties ) | No      | string | No         | -          | -                 |

### <a name="kmsKeyConsumerAccounts_additionalProperties"></a>3.1. Property `root > kmsKeyConsumerAccounts > additionalProperties`
...
## <a name="producerAccounts"></a>5. Property `root > producerAccounts`

|                           |                                                                                               |
| ------------------------- | --------------------------------------------------------------------------------------------- |
| **Type**                  | `object`                                                                                      |
| **Required**              | No                                                                                            |
| **Additional properties** | [Each additional property must conform to the schema](#producerAccounts_additionalProperties) |

**Description:** Producer accounts for which additional Athena data source catalogs are created in the deployment account.
...
| Property                                      | Pattern | Type   | Deprecated | Definition | Title/Description |
| --------------------------------------------- | ------- | ------ | ---------- | ---------- | ----------------- |
| - [](#producerAccounts_additionalProperties ) | No      | string | No         | -          | -                 |

### <a name="producerAccounts_additionalProperties"></a>5.1. Property `root > producerAccounts > additionalProperties`
